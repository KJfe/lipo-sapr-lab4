...
static int test_count_word_length()
{
	...
	put_value(&prog_ptr, 0x0500);				prog_counter += 2;	/*count_word_length(500)*/
...
static int test_string_concat()
{
	...
	put_commd(&prog_ptr, CALO);					prog_counter++;
	put_value(&prog_ptr, 0x0520);				prog_counter += 2;	/*string_concat(520)*/
	
	put_commd(&prog_ptr, CHALT);				prog_counter++;
	
	put_count_word_length(&prog_ptr, 0x0500, &link_table, &prog_counter, global_var);
	put_string_concat(&prog_ptr, 0x0520, &link_table, &prog_counter, global_var, 0x0500);
...
static int test_string_repeat()
{
	...
	put_commd(&prog_ptr, CALO);					prog_counter++;
	put_value(&prog_ptr, 0x0510);				prog_counter += 2;	/*string_repeat(510)*/
	
	put_commd(&prog_ptr, CHALT);				prog_counter++;
	
	put_count_word_length(&prog_ptr, 0x0500, &link_table, &prog_counter, global_var);
	put_string_repeat(&prog_ptr, 0x0510, &link_table, &prog_counter, global_var, 0x0500);
